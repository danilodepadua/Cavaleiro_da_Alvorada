...
public class CassinoController implements Initializable {
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        try {
            ...
            txtMoedas.setText(TextosInterface.getCarteira() + Player.getPlayer().getCoins() + " Makkos");
            JogoFachada.getInstance().getAudioPlayer().PlayLoop("/com.daniel.audios/msc_taverna.wav");
            Point2D centro = new Point2D(315, 169);
            desenharTexto(TextosInterface.getCassino(), 40, centro);
        } catch (PlayerInexistenteException e) {
            throw new RuntimeException(e);
        }
        btnMemoria.setText(TextosInterface.getMemoria());
        Platform.runLater(()->{
            Utilidades.AlinharHorizontal(vboxTextos,0.5);
            Utilidades.AlinharHorizontal(txtCassino,0.5);
        });
